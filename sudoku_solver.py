def main():
    ...
    while (True):
        ...
        # full dictionaries (len == 8) are replaced by correct numbers
...
# create a new nested list containing all squares and updating the dictionary in this nested list
...
# map results from the squares back to the original Sudoku format
...
# check if there are completely filled dictionaries (eight entries) and replace them by the respective number
...
# function for printing the Sudoku on the screen
...
# check whether there are still dictionaries remaining in the Sudoku
